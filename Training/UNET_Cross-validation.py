...
from unet_framework.utils.augmentations import CenterCrop, Dense_Target, AlbuSeg3d, ExpandDim, RandomFlip, Normalize01
from unet_framework.unet.unet import UNet
from unet_framework.loss.losses import DiceLoss
...
save_folder_name = 'UNET_Cross-validation'  # folder name
...
name_config = 'temp_config.py'  # config name
...
import unet_framework.data.Microtubules.BG_CM_MT.Training_data.Size_24x200x200.Input as input_dir
import unet_framework.data.Microtubules.BG_CM_MT.Training_data.Size_24x200x200.Target as target_dir

# Input and target directories
input_dir = input_dir.__path__._path[0]  # location of the input files
target_dir = target_dir.__path__._path[0]  # location of the target files
...
input_IDs = get_IDs_of_dir(input_dir, ext=input_ext)  # list of input files (Training)
target_IDs = get_IDs_of_dir(target_dir, ext=target_ext)  # list of target files (Training)
...
for seed in range(50):
    ...
    train_test_split_input = train_test_split(input_IDs,
                                              random_state=random_seed,
                                              train_size=train_size,
                                              shuffle=True
                                              )
    train_input_IDs = train_test_split_input[0]
    valid_input_IDs = train_test_split_input[1]

    train_test_split_target = train_test_split(target_IDs,
                                               random_state=random_seed,
                                               train_size=train_size,
                                               shuffle=True
                                               )
    train_target_IDs = train_test_split_target[0]
    valid_target_IDs = train_test_split_target[1]

    if valid_input_IDs[0] not in val:
        val.append(valid_input_IDs[0])
        seeds.append(seed)

from sklearn.model_selection import ParameterGrid

parameters = {
    'seed': seeds  # each seed has a unique fold
}
...
for param_set in parameter_grid:
    ...
    save_root_seed = os.path.join(save_root, f'{param_set}')

    # Seed
    random_seed = param_set['seed']
    ...
    # Loss
    criterion1 = torch.nn.CrossEntropyLoss()  # Cross entropy loss
    criterion1.weight = torch.tensor([0.05, 0.35, 0.60])  # Weight for cross entropy loss
    criterion2 = DiceLoss(apply_activation_function='softmax')  # Dice loss
    criterion2.weight = torch.tensor([0.05, 0.35, 0.60])  # Weight for dice loss
    ...
    # Early stopping
    early_stopping_params = {'mode': 'max', 'patience': 150, 'crit': 'valid_DSC_mean'}
    ...
    config_file = {
        ...
        'criterion2': criterion2,  # Optional
        'epochs': epochs,
        'update': update,
        'epoch': epoch,
