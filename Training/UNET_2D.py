...
from unet_framework.utils.augmentations import CenterCrop, Dense_Target, AlbuSeg2d, ExpandDim, RandomFlip, Normalize01, Random_Slice
...
save_folder_name = 'UNET_2D'  # folder name
...
import unet_framework.data.Microtubules.BG_CM_MT.Test_Validation_data.Input_2D as input_dir_validation
import unet_framework.data.Microtubules.BG_CM_MT.Test_Validation_data.Target_2D as target_dir_validation
...
transform_train_args = [
    ...
    Random_Slice(),  # Randomly selects a slice of the stack
    Dense_Target(),  # Ensure a dense integer representation
    AlbuSeg2d(albu=albumentations.ShiftScaleRotate(p=0.75,  # Wrapper for albumentations
                                                   rotate_limit=180,
                                                   scale_limit=0,
                                                   shift_limit=0.0625,
                                                   interpolation=3)),
    ...
    RandomFlip(ndim_spatial=2),  # Random 90° Flip along all spatial axes
